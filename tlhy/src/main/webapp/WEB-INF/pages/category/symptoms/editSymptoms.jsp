...
<html>
<head>
<title>Edit <fmt:message key="category.title"/> <fmt:message key="symptom.title"/></title>
</head>
<body>
<div id="contentarea">      
	...
	<div id="content">
		...
		<form:form action="${pageContext.request.contextPath}/saveCategorySymptoms" method="POST" modelAttribute="symptom">
				...
				<tbody>
					<tr>
						...
						<td>
							<c:choose>
								<c:when test='${newFlag}' >
							<form:input id="symptom_id" path="id" cssStyle="width:300px;"/>
							<script type="text/javascript">Spring.addDecoration(new Spring.ElementDecoration({elementId : "symptom_id",widgetType : "dijit.form.NumberTextBox",widgetAttrs : {promptMessage: "<fmt:message key="symptom.id.help"/>", constraints : {places:0}}})); </script>
								</c:when>
								<c:otherwise>
							${symptom.id}
						&nbsp;
